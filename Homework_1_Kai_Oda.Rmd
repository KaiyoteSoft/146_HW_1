...
birds <- birds[order(birds$fraction),]
...
[Fill in below:]
There are __43__ species in the dataset, but only __1__ accounted for 90% of the counts.
...
ggplot(endangered, aes(x=taxon, y=no.species))+
  geom_col(color=endangered$vertebrate, 
           fill=endangered$vertebrate)+
...
ggplot(mammals, aes(x=body_mass, y=brain_ratio))+
  geom_point()+
  geom_hline(yintercept=mean(mammals$brain_ratio))+
  geom_text(aes(label=name), size=3, vjust="inward", hjust="inward")
...
If the species is above the horizontal line than the animal has a higher brain-to-body mass ratio. However, if the species is below the horizontal line than the animal has a lower brain-to-body mass ratio. High ratio animals include primates such as monkeys as well as humans (arguably the most intelligent species in the dataset). However, there are some low ratio animals that are recognized as intelligent, like the Asian and African elephant. 
